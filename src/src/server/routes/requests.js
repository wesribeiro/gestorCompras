 ...
 * ATUALIZAÇÃO v4.5: Traz dados do Kanban (Status Real e ID do Pedido)
 */
router.get("/my", (req, res) => {
  ...
  const sql = `
        SELECT 
            pr.*,
            kp.id as pedido_id,
            kc.title as kanban_status,
            kc.color as kanban_color
        FROM PurchaseRequests pr
        LEFT JOIN KanbanPedidos kp ON pr.id = kp.request_id
