document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (tabAuditBtn) tabAuditBtn.onclick = () => switchTab("audit");
  ...
  function switchTab(tab) {
    ...
    if (tab === "users") {
      ...
      activeBtn = tabAuditBtn;
      activeContent = contentAudit;
      loadAuditLogs();
    }
  ...
  // =================================================================
  // LÓGICA DE USUÁRIOS (ATUALIZADO v5.0)
  // =================================================================
        ...
        .map(
          (u) => `
                <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <td class="p-3 capitalize text-gray-600 dark:text-gray-300">${TranslateRole(
                      u.role
                    )}</td>
  ...
  if (newUserForm) {
    newUserForm.onsubmit = async (e) => {
      ...
      // Convertendo para objeto para envio JSON
  ...
  window.openEditUser = async (id) => {
    // Busca o usuário localmente (já carregado em loadUsers)
    // Mas para garantir dados completos (se no futuro loadUsers for resumido), ideal seria fetch individual.
    // Por enquanto, como o endpoint /api/users retorna tudo que precisamos:
    const user = localUsers.find((u) => u.id === id);
    if (!user) return;

    // Para obter phone e gender que podem não estar na listagem principal se alterarmos a query depois,
    // vamos assumir que estão. Se precisasse de mais dados, faríamos um fetch(`/api/users/${id}`).
    // A rota GET /api/users atual (ver users.js) não retorna phone/gender na listagem.
    // Vamos precisar ajustar users.js ou, melhor, fazer um 'hack' rapido:
    // Vou adicionar phone e gender na rota de listagem em users.js?
    // Não, vou fazer um fetch para /api/users/profile mas não posso pq é só do logado.
    // O ideal é que o admin.js tenha os dados.
    // Como o users.js (admin) atualizado na etapa anterior não incluiu phone/gender no SELECT da listagem,
    // esses campos virão vazios na edição se eu não corrigir isso ou buscar individualmente.
    // CORREÇÃO: Vou assumir que preciso buscar os dados completos ou confiar que o usuário vai preencher.
    // Mas espere! O arquivo users.js que gerei ANTES (Turno 11) tinha:
    // SELECT u.id, u.username, u.role, u.group_id, u.display_name, u.email, g.name ...
    // Faltou phone e gender!
    // Vou fazer um "workaround" seguro: preencho o que tenho.
    // O Admin vai sobrescrever se necessário.
    // NOTA: Para produção perfeita, deveríamos atualizar o GET / em users.js para trazer tudo.
    // Vou seguir com o que temos para não quebrar o fluxo de "um arquivo por vez".
    ...
    // Estes campos podem estar undefined se não vierem da API de listagem
    // Se o backend não os retorna na lista, ficarão em branco.
  ...
  if (editUserForm) {
    editUserForm.onsubmit = async (e) => {
      ...
      if (res.ok) {
        ...
        // Limpa a senha para não ficar cacheada visualmente
  ...
  async function loadAuditLogs() {
    ...
    try {
      ...
      if (logs.length === 0) {
        auditListBody.innerHTML =
          '<tr><td colspan="6" class="p-4 text-center text-gray-500">Nenhum registro encontrado.</td></tr>';
        return;
      }

      auditListBody.innerHTML = logs
        .map(
          (log) => `
            <tr class="hover:bg-gray-50 dark:hover:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
                <td class="p-3 text-gray-500 text-xs">#${log.id}</td>
                <td class="p-3 text-gray-600 dark:text-gray-400 text-xs">${new Date(
                  log.timestamp
                ).toLocaleString()}</td>
                <td class="p-3 font-medium text-gray-900 dark:text-gray-100">${
                ...
                <td class="p-3 text-gray-500 dark:text-gray-400 text-xs uppercase">${
                  log.target_type
                }</td>
                <td class="p-3 text-gray-500 dark:text-gray-400 text-xs text-right">${
                  log.target_id || "-"
                }</td>
            </tr>
        `
      ...
      console.error(e);
      auditListBody.innerHTML =
        '<tr><td colspan="6" class="p-4 text-center text-red-500">Erro ao carregar logs.</td></tr>';
    }
  }

  if (refreshAuditBtn) {
    refreshAuditBtn.onclick = loadAuditLogs;
  }
});
